...
@Injectable()
export class PrebookingHotelsRepository
  implements GenericRepository<PrebookingHotel>
{
  constructor(
    @InjectRepository(PrebookingHotel)
    private readonly repository: Repository<PrebookingHotel>,
    @Inject(REQUEST) private readonly request: RequestWithUser,
    private readonly loggerService: MyLoggerService,
  ) {}

  async create(entity: PrebookingHotel) {
  ...
  async fetchById(id: number): Promise<PrebookingHotel> {
  ...
  async update(entity: PrebookingHotel) {
