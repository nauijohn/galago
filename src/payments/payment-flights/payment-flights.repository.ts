  ...
  implements GenericRepository<PaymentFlight>
{
  ...
  async create(entity: PaymentFlight) {
  ...
  async update(entity: PaymentFlight) {
  ...
  async findPaymentIntentById(paymentIntentId: string) {
    ...
    return await this.repository.findOneBy({ paymentIntentId });
  }
  ...
  async fetchAllByPromoCode(promoCode: string) {
    ...
    return await this.repository.find({
      where: {
        userId: this.request.user?.id ?? null,
        promoCodes: ArrayContains([promoCode]),
      },
    });
  }
